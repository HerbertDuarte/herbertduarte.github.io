...
<template>
  <section id="skills">
    ...
    <p class="py-10">
      <BrainIcon :stroke-width="2.2" :size="32" class="inline text-primary pb-1.5" />
      Conheça um pouco mais sobre minhas habilidades e tecnologias que domino,
      envolvendo tanto o desenvolvimento front-end quanto o back-end.
    </p>
    <div class="flex lg:flex-row flex-col lg:gap-8 gap-0">
      <div class="flex-1 overflow-hidden">
        <SkillBarProgress v-for="(item, index) in skillsArrays[0]" :key="index" :name="item.name" :level="item.level" />
      </div>
      <div class="flex-1 overflow-hidden">
        <SkillBarProgress v-for="(item, index) in skillsArrays[1]" :key="index" :name="item.name" :level="item.level" />
      </div>
    </div>
  </section>
</template>
